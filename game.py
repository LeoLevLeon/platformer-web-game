...
class Button():
    def __init__(self, x, y, color_1, color_2, text=''):
        ...
        self.normal_color = color_1
        self.hover_color = color_2
        self.text_color = (225, 225, 225)
        
        # Больше размер для пальцев!
        self.rect = pygame.Rect(x, y, 180, 120)
        self.text = text
        self.font = pygame.font.SysFont(None, 28)  # Шрифт побольше
        self.is_hovered = False
        
        # Для мобильных: всегда "подсвеченная"
        self.always_highlight = False
        
    def draw_button(self, surface):
        # Если мобильное устройство, используем hover-цвет
        if self.always_highlight or self.is_hovered:
    ...
    def update_hover(self, mouse_pos):
        # Для веб-версии: если это мобильное, hover не работает
        try:
            import js
            if hasattr(js.window, 'ontouchstart'):
                self.always_highlight = True
                return
        except:
            pass
...
class FakeWebbrowser:
    def open(self, url):
        print(f"Открываем ссылку в браузере: {url}")
        try:
            import js
...
# ========== УПРАВЛЕНИЕ ДЛЯ ВЕБ-ВЕРСИИ ==========
def check_web_controls():
    """Проверяет сенсорное управление из JavaScript"""
    try:
        import js
        # Возвращаем словарь с состоянием кнопок
        return {
        ...
        # Если не в браузере или ошибка
        return {'left': False, 'right': False, 'jump': False}

def handle_input(player, jump_sound_obj, sound_enabled):
    """
    Обрабатывает все типы управления
    
    Аргументы:
    - player: объект игрока (player_sprite)
    - jump_sound_obj: объект звука прыжка
    ...
    # 1. КЛАВИАТУРА (для ПК и браузера с клавиатурой)
    ...
    # 2. СЕНСОРНОЕ УПРАВЛЕНИЕ (только в браузере)
    
    controls = check_web_controls()
    ...
    # Прыжок (обрабатываем только в момент нажатия)
...
button_1 = Button(300, 200, "#c84420", "#f27552", "Старт")
button_2 = Button(300, 350, "#2067c8", "#528af2", "Полный экран")
button_3 = Button(300, 500, "#30c820", "#52f25d", "Звук вкл/выкл")
